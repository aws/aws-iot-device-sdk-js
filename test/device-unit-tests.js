...
//
   ...
   describe( "device does not throw an exception if using govcloud host", function() {
      //
      // Verify that the device module does not throw an exception when configured for
      // websocket operation with gov cloud endpoint
      //
            it("does not throw an exception", function() {
               ...
               assert.doesNotThrow( function( err ) {
                  ...
                  process.env.AWS_SECRET_ACCESS_KEY='not a valid secret access key';
                  var device = deviceModule( {
                     host:'XXXX.iot.us-gov-east-1.amazonaws.com',
                     protocol: 'wss',
                     debug: true
                     } );
                  }, function(err) { console.log('\t['+err+']'); return true;}
