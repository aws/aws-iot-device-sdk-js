...
function ThingShadowsClient(deviceOptions, thingShadowOptions) {
  ...
  //
  this._handleSubscriptions = function(
    thingName,
    topicSpecs,
    devFunction,
    callback
    ...
    for (var i = 0, topicsLen = topicSpecs.length; i < topicsLen; i++) {
      for (
        var j = 0, opsLen = topicSpecs[i].operations.length;
        j < opsLen;
        j++
      ) {
        for (
          var k = 0, statLen = topicSpecs[i].statii.length;
          k < statLen;
          k++
        ) {
          topics.push(
            buildThingShadowTopic(
              thingName,
  ...
  this._handleMessages = function(
    ...
    try {
      ...
      if (deviceOptions.debug === true) {
        console.error(
          "failed parsing JSON '" + payload.toString() + "', " + err
        );
      }
      return;
    }
    ...
    if (!isUndefined(version) && operationStatus !== 'rejected') {
      ...
      //
      if (
        isUndefined(thingShadows[thingName].version) ||
        ...
        if (
          operation !== 'delete' &&
          thingShadows[thingName].discardStale === true
        ) {
          if (deviceOptions.debug === true) {
            console.warn(
              "out-of-date version '" +
    ...
    //
    if (
      isUndefined(thingShadows[thingName].clientToken) ||
    ...
    if (connected === true) {
      ...
      if (isThingShadowTopic(topicTokens, 'subscribe')) {
        ...
        if (thingShadows.hasOwnProperty(topicTokens[2])) {
          ...
          //
          that._handleMessages(
            topicTokens[2], // thingName
  ...
  this._thingOperation = function(thingName, operation, stateObject) {
    ...
    if (thingShadows.hasOwnProperty(thingName)) {
      ...
      if (thingShadows[thingName].pending === false) {
        ...
        if (isUndefined(stateObject.clientToken)) {
          ...
          if (clientIdLength > 48) {
            clientToken =
              deviceOptions.clientId.substr(clientIdLength - 48) +
              '-' +
              operationCount++;
          } else {
          ...
          this._handleSubscriptions(
            ...
            function(err, failedTopics) {
              ...
              if (!isUndefined(stateObject)) {
                ...
                if (
                  !isUndefined(thingShadows[thingName]) &&
      ...
      if (deviceOptions.debug === true) {
        console.error(
          'attempting to ' + operation + ' unknown thing: ',
          thingName
        );
      }
    }
    ...
    if (!thingShadows.hasOwnProperty(thingName)) {
      ...
      if (!isUndefined(options)) {
        ...
        if (!isUndefined(options.persistentSubscribe)) {
          thingShadows[thingName].persistentSubscribe =
      ...
      if (topicSpecs.length > 0) {
        this._handleSubscriptions(thingName, topicSpecs, 'subscribe', function(
          err,
          failedTopics
        ) {
          if (isUndefined(err) && isUndefined(failedTopics)) {
  ...
  this.subscribe = function(topics, options, callback) {
    ...
    for (var i = 0; i < topicsArray.length; i++) {
      if (isReservedTopic(topicsArray[i])) {
        throw "cannot subscribe to topic array since one of them is a reserved topic '" +
          topicsArray[i] +
          "'";
      }
    }
  ...
  this.unsubscribe = function(topics, callback) {
    ...
    for (var i = 0; i < topicsArray.length; i++) {
      if (isReservedTopic(topicsArray[i])) {
        throw "cannot unsubscribe from topic array since one of them is a reserved topic '" +
          topicsArray[i] +
          "'";
      }
    }
  ...
  // to the device class.
  //
  this.updateWebSocketCredentials = function(
    accessKeyId,
    secretKey,
    sessionToken,
    expiration
