...
function commandHandler(job) {
    if (job.status.status === 'QUEUED' ||
        ...
        job.inProgress({ operation: job.operation, step: 'attempting' }, function (err) {
            ...
            // Prepare command.
            var command = (isUndefined(job.document.command) ? 'echo "thingname:{{thingname}}"' : job.document.command.toString());
            command = command.replace('{{thingname}}', context.thingName);
        ...
        job.failed({ operation: job.operation, errorCode: 'ERR_UNEXPECTED', errorMessage: 'command job execution in unexpected state' }, showJobsError);
    }
}
...
function jobsAgent(args) {
   ...
   subscribeToJobsWithRetryOnError(args.thingName, 'command', commandHandler);
